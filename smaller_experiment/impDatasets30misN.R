...
load("./data/results/wave1imp-30-n.RData")
...
impNets.60.2.30.n.t1 <- list()
impAlco.60.2.30.n.t1 <- list()

impNets.60.2.30.n.t2 <- list()
impAlco.60.2.30.n.t2 <- list()

saom.results.30.n.t1 <- list()
saom.results.30.n.t2 <- list()

fr.60.2.sim.mis.30.n[,,3] <- fr.60.2.sim.mis.30.n[,,19]
alco.60.2.sim.mis.30.n[,3] <- alco.60.2.sim.mis.30.n[,19]
...
for (t in 1:2) { 
  for (d in 1:D) {
    ...
    friendship <- sienaDependent(array(c(net1imp.t1[[d]],
                                        fr.60.2.sim.mis.30.n[,,i]),
                                      dim = c(N,N,M)))
    
    drinkingbeh <- sienaDependent(cbind(alc1imp.t1[,d],
                                        alco.60.2.sim.mis.30.n[,i]),
                                                  type = "behavior")
    } else {
    ...
    if (t == 1) {
      ...
      net2imp.t1[[d]] <- getNet(fr.60.2.sim.mis.30.n[,,i], sims[[1]])
      alc2imp.t1[,d] <- sims[[2]]
    } else {
      effects.twoWaves <- setEffect(effects.twoWaves, egoX,
                                    name = "friendship",
      ...
      net2imp.t2[[d]] <- getNet(fr.60.2.sim.mis.30.n[,,i], sims[[1]])
      alc2imp.t2[,d] <- sims[[2]]
    }
...
file = "./data/results/30misN_before_estimation.RData")
...
for (t in 1:2) {
  ...
  save(effects.imputed,
       saom.results.30.n.t1, saom.results.30.n.t2, # save after each dataset imp
       file = './data/results/result-30-n.RData')
