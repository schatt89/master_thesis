...
fr.1.mis.20.nb[to_remove.1,] <- NA
alco.1.mis.20.nb[to_remove.1] <- NA
...
for (i in 1:S) {
  ...
  fr.60.2.sim.mis.20.nb[,,i][to_remove.2,] <- NA
  alco.60.2.sim.mis.20.nb[,i][to_remove.2] <- NA
...
########   fr.60.2.sim.mis.20.nb, alco.60.2.sim.mis.20.nb

miceImpAlco.60.1.20.nb <- array(rep(NA, N*D), c(N, D))

indegree1 <- colSums(fr.1.mis.20.nb, na.rm = TRUE)
indegree2 <- colSums(fr.60.2.sim.mis.20.nb[,,i], na.rm = TRUE)

avgAltA1 <- rowSums(sweep(t(fr.1.mis.20.nb),
                          MARGIN = 2, alco.1.mis.20.nb,'*'), na.rm = TRUE) /
  rowSums(t(fr.1.mis.20.nb), na.rm = TRUE)

avgAltA2 <- rowSums(sweep(t(fr.60.2.sim.mis.20.nb[,,i]),
                          MARGIN = 2, alco.60.2.sim.mis.20.nb[,i],'*'), na.rm = TRUE) /
  rowSums(t(fr.60.2.sim.mis.20.nb[,,i]), na.rm = TRUE)
...
miceData <- cbind(alco.1.mis.20.nb, alco.60.2.sim.mis.20.nb[,i],
                  indegree1, indegree2,
                  avgAltA1, avgAltA2)
...
for (d in 1:D) {
  miceImpAlco.60.1.20.nb[,d] <- complete(miceImp, d)$alco.1.mis.20.nb
}

friendship <- sienaDependent(array(c(fr.1.mis.20.nb,
                                     fr.1.mis.20.nb),
                                   dim = c(N, N, M)) ,
                             allowOnly = FALSE)

w2 <- coDyadCovar(fr.60.2.sim.mis.20.nb[,,i]) # the 2nd wave incomplete
# network as covariate
a2 <- coCovar(alco.60.2.sim.mis.20.nb[,i]) # the 2nd wave incomplete
...
for (d in 1:D) {
  drinkingbeh <- sienaDependent(cbind(miceImpAlco.60.1.20.nb[,d],
                                      miceImpAlco.60.1.20.nb[,d]), 
...
for(t in 1:2) {
  ...
  for (d in 1:D) {
    ...
    a1c <- sample(a1.3s,1)
    a1change <- miceImpAlco.60.1.20.nb[,d]
  ...
  if (t == 1) {
    for (d in 1:D) {
      net1imp.t1[[d]] = getNet(fr.1.mis.20.nb, sims[[d]][[1]][[1]])
      alc1imp.t1[,d] = sims[[d]][[2]][[1]]
      
      changed <- which(((alco.1.mis.20.nb - alc1imp.t1[,d]) != 0))[[1]]
      alc1imp.t1[,d][changed] <- alco.1.mis.20.nb[changed]
    }
  } else {
    for (d in 1:D) {
      net1imp.t2[[d]] = getNet(fr.1.mis.20.nb, sims[[d]][[1]][[1]])
      alc1imp.t2[,d] = sims[[d]][[2]][[1]]
      
      changed <- which(((alco.1.mis.20.nb - alc1imp.t2[,d]) != 0))[[1]]
      alc1imp.t2[,d][changed] <- alco.1.mis.20.nb[changed]
    }
  }
}

save(net1imp.t1, alc1imp.t1, net1imp.t2, alc1imp.t2,
     fr.60.2.sim.mis.20.nb,
     alco.60.2.sim.mis.20.nb,
     missing.20.2,
     missing.20.2.inv,
     file = "./data/results/wave1imp-20-nb.RData")