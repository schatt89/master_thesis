...
load("./data/results/wave1imp-20-nb.RData")
...
impNets.60.2.20.nb.t1 <- list()
impAlco.60.2.20.nb.t1 <- list()

impNets.60.2.20.nb.t2 <- list()
impAlco.60.2.20.nb.t2 <- list()
...
for (t in 1:2) { 
  for (d in 1:D) {
    ...
    friendship <- sienaDependent(array(c(net1imp.t1[[d]],
                                        fr.60.2.sim.mis.20.nb[,,i]),
                                      dim = c(N,N,M)))
    
    drinkingbeh <- sienaDependent(cbind(alc1imp.t1[,d],
                                        alco.60.2.sim.mis.20.nb[,i]),
                                                  type = "behavior")
    } else {
    ...
    if (t == 1) {
      ...
      net2imp.t1[[d]] <- getNet(fr.60.2.sim.mis.20.nb[,,i], sims[[1]][[1]]$`1`)
      ...
      net2imp.t2[[d]] <- getNet(fr.60.2.sim.mis.20.nb[,,i], sims[[1]][[1]]$`1`)
...
for (t in 1:2) {
  ...
  saom.results.20.nb.t1[[i]] <- list(thetas.t1, covthetas.t1)
  saom.results.20.nb.t2[[i]] <- list(thetas.t2, covthetas.t2)
  
  save(effects.imputed,
       saom.results.20.nb.t1, saom.results.20.nb.t2, # save after each dataset imp
       file = './data/results/result-20-nb.RData')
