...
load("./data/results/wave1imp-30-b.RData")
...
impNets.60.2.30.b.t1 <- list()
impAlco.60.2.30.b.t1 <- list()

impNets.60.2.30.b.t2 <- list()
impAlco.60.2.30.b.t2 <- list()

saom.results.30.b.t1 <- list()
saom.results.30.b.t2 <- list()
...
for (t in 1:2) { 
  for (d in 1:D) {
    ...
    friendship <- sienaDependent(array(c(net1imp.t1[[d]],
                                        fr.60.2.sim.mis.30.b[,,i]),
                                      dim = c(N,N,M)))
    
    drinkingbeh <- sienaDependent(cbind(alc1imp.t1[,d],
                                        alco.60.2.sim.mis.30.b[,i]),
                                                  type = "behavior")
    } else {
    ...
    if (t == 1) {
      ...
      net2imp.t1[[d]] <- getNet(fr.60.2.sim.mis.30.b[,,i], sims[[1]][[1]]$`1`)
      ...
      net2imp.t2[[d]] <- getNet(fr.60.2.sim.mis.30.b[,,i], sims[[1]][[1]]$`1`)
...
for (t in 1:2) {
  ...
  saom.results.30.b.t1[[i]] <- list(thetas.t1, covthetas.t1)
  saom.results.30.b.t2[[i]] <- list(thetas.t2, covthetas.t2)
  
  save(effects.imputed,
       saom.results.30.b.t1, saom.results.30.b.t2, # save after each dataset imp
       file = './data/results/result-30-b.RData')
