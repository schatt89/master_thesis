...
# Example call:
for (i in 1:S) { 
    SN <- SimulateNetworksBehavior(n, M, c, rate, dens, rec, tt, c3, Vaego,
                                Vaalt, Vasim, Vbego, Vbalt, Vbsim, rate.b,
                                lin.b, qu.b, avalt.b)

  fr.30.1.sim[,,i] <- SN$networks[,,1]
  fr.30.2.sim[,,i] <- SN$networks[,,2]

  alco.30.1.sim[,i] <- SN$behavior[,1]
  alco.30.2.sim[,i] <- SN$behavior[,2]
}

covara <- SN$covara
covarb <- SN$covarb

### 10% missing data depending on the network ####
...
for (i in 1:S) {
  to_remove.1 <- sample(which(rowSums(fr.30.1.sim[,,i]) < 3), 3, replace=F)
  to_remove.2 <- sample(which(rowSums(fr.30.2.sim[,,i]) < 3), 3, replace=F)
  for (j in 1:3) {
...
for (i in 1:S) {
    ...
    covarA <- coCovar(covara)
    covarB <- coCovar(covarb)
    ...
    for (d in 1:D) {
        effects.stationary <- setEffect(effects.stationary, Rate,
                                    initialValue = 5,
...
########################### later waves imputation ###########################
    ...
    for (d in 1:D) {
        ...
        Data.w2  <- sienaDataCreate(friendship, drinkingbeh, covarA, covarB)
