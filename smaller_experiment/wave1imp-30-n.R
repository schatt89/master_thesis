...
########   fr.60.2.sim.mis.30.n, alco.60.2.sim.mis.30.n

miceImpAlco.60.1.30.n <- array(rep(NA, N*D), c(N, D))

indegree1 <- colSums(fr.1.mis.30.n, na.rm = TRUE)
indegree2 <- colSums(fr.60.2.sim.mis.30.n[,,i], na.rm = TRUE)

avgAltA1 <- rowSums(sweep(t(fr.1.mis.30.n),
                          MARGIN = 2, alco.1.mis.30.n,'*'), na.rm = TRUE) /
  rowSums(t(fr.1.mis.30.n), na.rm = TRUE)

avgAltA2 <- rowSums(sweep(t(fr.60.2.sim.mis.30.n[,,i]),
                          MARGIN = 2, alco.60.2.sim.mis.30.n[,i],'*'), na.rm = TRUE) /
  rowSums(t(fr.60.2.sim.mis.30.n[,,i]), na.rm = TRUE)
...
miceData <- cbind(alco.1.mis.30.n, alco.60.2.sim.mis.30.n[,i],
                  indegree1, indegree2,
                  avgAltA1, avgAltA2)
...
friendship <- sienaDependent(array(c(fr.1.mis.30.n,
                                     fr.1.mis.30.n),
                                   dim = c(N, N, M)) ,
                             allowOnly = FALSE)

w2 <- coDyadCovar(fr.60.2.sim.mis.30.n[,,i]) # the 2nd wave incomplete
# network as covariate
a2 <- coCovar(alco.60.2.sim.mis.30.n[,i]) # the 2nd wave incomplete
...
for (d in 1:D) {
  drinkingbeh <- sienaDependent(cbind(miceImpAlco.60.1.30.n[,d],
                                      miceImpAlco.60.1.30.n[,d]), 
...
for(t in 1:2) {
  ...
  for (d in 1:D) {
    ...
    a1c <- sample(a1.3s,1)
    a1change <- miceImpAlco.60.1.30.n[,d]
  ...
  if (t == 1) {
    for (d in 1:D) {
      net1imp.t1[[d]] = getNet(fr.1.mis.30.n, sims[[d]][[1]][[1]])
      alc1imp.t1[,d] = sims[[d]][[2]][[1]]
      
      changed <- which(((alco.1.mis.30.n - alc1imp.t1[,d]) != 0))[[1]]
      alc1imp.t1[,d][changed] <- alco.1.mis.30.n[changed]
    }
  } else {
    for (d in 1:D) {
      net1imp.t2[[d]] = getNet(fr.1.mis.30.n, sims[[d]][[1]][[1]])
      alc1imp.t2[,d] = sims[[d]][[2]][[1]]
      
      changed <- which(((alco.1.mis.30.n - alc1imp.t2[,d]) != 0))[[1]]
      alc1imp.t2[,d][changed] <- alco.1.mis.30.n[changed]
    }
  }
}

save(net1imp.t1, alc1imp.t1, net1imp.t2, alc1imp.t2,
     fr.60.2.sim.mis.30.n,
     alco.60.2.sim.mis.30.n,
     missing.30.2,
     missing.30.2.inv,
