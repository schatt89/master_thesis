...
fr.1.mis.30.mcar <- fr.1
alco.1.mis.30.mcar <- alco[,1]
...
fr.1.mis.30.mcar[to_remove.1,] <- NA
alco.1.mis.30.mcar[to_remove.1] <- NA


fr.60.2.sim.mis.30.mcar <- fr.60.2.sim
alco.60.2.sim.mis.30.mcar <- alco.60.2.sim
...
for (i in 1:S) {
  ...
  fr.60.2.sim.mis.30.mcar[,,i][to_remove.2,] <- NA
  alco.60.2.sim.mis.30.mcar[,i][to_remove.2] <- NA
...
miceImpAlco.60.1.30.mcar <- array(rep(NA, N*D), c(N, D))

indegree1 <- colSums(fr.1.mis.30.mcar, na.rm = TRUE)
indegree2 <- colSums(fr.60.2.sim.mis.30.mcar[,,i], na.rm = TRUE)

avgAltA1 <- rowSums(sweep(t(fr.1.mis.30.mcar),
                          MARGIN = 2, alco.1.mis.30.mcar,'*'), na.rm = TRUE) /
  rowSums(t(fr.1.mis.30.mcar), na.rm = TRUE)

avgAltA2 <- rowSums(sweep(t(fr.60.2.sim.mis.30.mcar[,,i]),
                          MARGIN = 2, alco.60.2.sim.mis.30.mcar[,i],'*'), na.rm = TRUE) /
  rowSums(t(fr.60.2.sim.mis.30.mcar[,,i]), na.rm = TRUE)



avgAltA1[is.na(avgAltA1)] <- NA
avgAltA2[is.na(avgAltA2)] <- NA


miceData <- cbind(alco.1.mis.30.mcar, alco.60.2.sim.mis.30.mcar[,i],
                  indegree1, indegree2,
                  avgAltA1, avgAltA2)
...
for (d in 1:D) {
  miceImpAlco.60.1.30.mcar[,d] <- complete(miceImp, d)$alco.1.mis.30.mcar
}

friendship <- sienaDependent(array(c(fr.1.mis.30.mcar,
                                     fr.1.mis.30.mcar),
                                   dim = c(N, N, M)) ,
                             allowOnly = FALSE)

w2 <- coDyadCovar(fr.60.2.sim.mis.30.mcar[,,i]) # the 2nd wave incomplete
# network as covariate
a2 <- coCovar(alco.60.2.sim.mis.30.mcar[,i]) # the 2nd wave incomplete
...
for (d in 1:D) {
  drinkingbeh <- sienaDependent(cbind(miceImpAlco.60.1.30.mcar[,d],
                                      miceImpAlco.60.1.30.mcar[,d]), 
...
a1 <- alco.1.mis.30.mcar
a1.3s <- c(1:N)[a1 == 3 & !is.na(a1)]
a1c <- sample(a1.3s,1)
a1change <- miceImpAlco.60.1.30.mcar[,d]
...
for (d in 1:D) {
    net1imp[[d]] = getNet(fr.1.mis.30.mcar, sims[[d]][[1]][[1]])
    alc1imp[,d] = sims[[d]][[2]][[1]]
    
    changed <- which(((alco.1.mis.30.mcar - alc1imp[,d]) != 0))[[1]]
    alc1imp[,d][changed] <- alco.1.mis.30.mcar[changed]
}



save(net1imp, alc1imp, 
     fr.60.2.sim.mis.30.mcar,
     alco.60.2.sim.mis.30.mcar,
     missing.30.2,
     missing.30.2.inv,
     file = "./data/results/wave1imp-30-mcar.RData")
